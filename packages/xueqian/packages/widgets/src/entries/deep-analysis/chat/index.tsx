...
import { useDeepAnalysisState } from '../model.js'
...
export function Chat(props) {
  ...
  const {
    ...
    append,
    error
  } = useDeepAnalysisState({
    endpoint: `/ai-agent/ws/chat?env=${props.env}&view=${props.view}&preset=${props.preset || ''}`
  });
      ...
      setMessages([
        {
          id: "",
          role: "assistant",
          content: "Hi👋，我是数据分析小助手，可以帮你深度分析数据和创建报告",
        },
      ]);
    }
  }, [messages]);
    ...
    <div className="w-screen h-screen overflow-hidden flex flex-col bg-white">
      <header className="bg-blue-600 flex flex-col items-center justify-center py-5 shrink-0">
        ...
        {!!props.name && (
          <p className="text-white text-xs mt-2">
            当前视图: {decodeURIComponent(props.name)}
          </p>
        )}
      </header>
      ...
      <footer className="shrink-0 px-4">
        ...
        <div className="border border-l-0 border-r-0 border-b-0 border-gray-100 pt-2 h-28 relative">
          ...
          />
          <div
            className="hidden bottom-4 left-0 p-2 cursor-pointer rounded-md hover:bg-gray-100"
            onClick={() => {
              if (loading) {
                return;
              }
